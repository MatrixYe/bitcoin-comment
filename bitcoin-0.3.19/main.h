...
static const unsigned int MAX_BLOCK_SIZE_GEN =
    MAX_BLOCK_SIZE / 2; // 生成新区块时的默认最大大小 (0.5MB)
static const int MAX_BLOCK_SIGOPS =
    MAX_BLOCK_SIZE / 50; // 区块中允许的最大签名操作数

static const int64 COIN = 100000000; // 1比特币 = 1亿聪 (最小货币单位)
static const int64 CENT = 1000000;   // 1分比特币 = 100万聪
static const int64 MAX_MONEY =
    21000000 * COIN; // 比特币最大供应量 (2100万比特币)
...
static const int COINBASE_MATURITY =
    100; // Coinbase交易成熟度要求 (需要100个确认才能使用)
...
class CInPoint {
public:
  CTransaction *ptx; // 上一个交易的指针
  unsigned int n;    // 上一个交易的输出索引

  CInPoint() { SetNull(); } // 默认构造函数，将指针和索引设置为NULL

  /**
   * 构造函数
   * @param ptxIn 上一个交易的指针
   * @param nIn 上一个交易的输出索引
   */
...
public:
  ...
  /**
   * 小于运算符重载
   * 用于支持在容器中排序和比较，先按交易哈希排序，再按输出索引排序
   */
