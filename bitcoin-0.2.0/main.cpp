...
bool CTransaction::AcceptTransaction(CTxDB &txdb, bool fCheckInputs,
  ...
  // To help v0.1.5 clients who would see it as negative number. please delete
  // this later.
...
bool CheckDiskSpace(int64 nAdditionalBytes) {
  ...
  if (nFreeBytesAvailable < (int64)15000000 + nAdditionalBytes) {
    fShutdown = true;
    ThreadSafeMessageBox("Warning: Your disk space is low  ", "Bitcoin",
...
void PrintBlockTree() {
  ...
  while (!vStack.empty()) {
    ...
    printf("%d (%u,%u) %s  %s  tx %d", pindex->nHeight, pindex->nFile,
           pindex->nBlockPos, block.GetHash().ToString().substr(0, 16).c_str(),
  ...
  loop {
    ...
    try {
      ...
      if (strstr(e.what(), "CDataStream::read() : end of data")) {
        ...
        printf("ProcessMessage(%s, %d bytes) : Exception '%s' caught, normally "
...
bool ProcessMessage(CNode *pfrom, string strCommand, CDataStream &vRecv) {
  ...
  if (strCommand == "version") {
    ...
    printf("getblocks %d to %s limit %d\n", (pindex ? pindex->nHeight : -1),
           hashStop.ToString().substr(0, 16).c_str(), nLimit);
...
bool SendMessages(CNode *pto) {
  CRITICAL_BLOCK(cs_main) {
    ...
    // Keep-alive ping
    if (pto->nLastSend && GetTime() - pto->nLastSend > 12 * 60 &&
        pto->vSend.empty())
...
bool CreateTransaction(CScript scriptPubKey, int64 nValue, CWalletTx &wtxNew,
                       CKey &keyRet, int64 &nFeeRequiredRet) {
  nFeeRequiredRet = 0;
  CRITICAL_BLOCK(cs_main) {
