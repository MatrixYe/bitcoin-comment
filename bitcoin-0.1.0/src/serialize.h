...
public:
  ...
  template <typename Stream> void Unserialize(Stream &s, int, int = 0) {
    if (pstr == NULL)
      throw std::ios_base::failure("CFixedFieldString::Unserialize : trying to "
                                   "unserialize to const string");
    char pszBuf[LEN + 1];
    s.read(pszBuf, LEN);
    pszBuf[LEN] = '\0';
    *pstr = pszBuf;
...
extern inline unsigned int GetSerializeSize(const CScript &v, int nType,
                                            int nVersion = VERSION);
template <typename Stream>
void Serialize(Stream &os, const CScript &v, int nType, int nVersion = VERSION);
template <typename Stream>
void Unserialize(Stream &is, CScript &v, int nType, int nVersion = VERSION);
...
// function. "int nType" is changed to "long nType" to keep from getting an
// ambiguous overload error. The compiler will only cast int to long if none of
// the other templates matched. Thanks to Boost serialization for this idea.
//
template <typename T>
...
public:
  ...
  CDataStream(const_iterator pbegin, const_iterator pend, int nTypeIn = 0,
              int nVersionIn = VERSION)
      : vch(pbegin, pend) {
    Init(nTypeIn, nVersionIn);
  }

#if !defined(_MSC_VER) || _MSC_VER >= 1300
  CDataStream(const char *pbegin, const char *pend, int nTypeIn = 0,
              int nVersionIn = VERSION)
      : vch(pbegin, pend) {
    Init(nTypeIn, nVersionIn);
  }
  ...
  CDataStream(const vector_type &vchIn, int nTypeIn = 0,
              int nVersionIn = VERSION)
      : vch(vchIn.begin(), vchIn.end()) {
    Init(nTypeIn, nVersionIn);
  }

  CDataStream(const vector<char> &vchIn, int nTypeIn = 0,
              int nVersionIn = VERSION)
      : vch(vchIn.begin(), vchIn.end()) {
    Init(nTypeIn, nVersionIn);
  }

  CDataStream(const vector<unsigned char> &vchIn, int nTypeIn = 0,
              int nVersionIn = VERSION)
...
public:
  ...
  CAutoFile &read(char *pch, int nSize) {
    ...
    if (fread(pch, 1, nSize, file) != nSize)
      setstate(ios::failbit, feof(file) ? "CAutoFile::read : end of file"
