...
CDB::CDB(const char *pszFile, const char *pszMode, bool fTxn) : pdb(NULL) {
  ...
  CRITICAL_BLOCK(cs_db) {
    if (!fDbEnvInit) {
      ...
      if (ret > 0)
        throw runtime_error(
            strprintf("CDB() : error %d opening database environment\n", ret));
      fDbEnvInit = true;
    }
...
bool CTxDB::ReadOwnerTxes(uint160 hash160, int nMinHeight,
    ...
    loop {
      ...
      if (strType == "name") {
        ...
        ssValue >> vchDefaultKeyRet;
                 ...
                 "setting") /// or settings or option or options or config?
      {
        string strKey;
        ssKey >> strKey;
