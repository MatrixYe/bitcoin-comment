...
string DateTimeStr(int64 nTime) {
  ...
  if (Is24HourTime())
    ...
    return (string)datetime.Format("%x ") +
           itostr((datetime.GetHour() + 11) % 12 + 1) +
           (string)datetime.Format(":%M %p");
}
...
int InsertLine(wxListCtrl *listCtrl, const wxString &str0,
               const wxString &str1) {
...
int InsertLine(wxListCtrl *listCtrl, const wxString &str0, const wxString &str1,
               const wxString &str2, const wxString &str3,
               const wxString &str4) {
  int nIndex = listCtrl->InsertItem(listCtrl->GetItemCount(), str0);
  listCtrl->SetItem(nIndex, 1, str1);
...
int InsertLine(wxListCtrl *listCtrl, void *pdata, const wxString &str0,
               const wxString &str1, const wxString &str2, const wxString &str3,
               const wxString &str4) {
  int nIndex = listCtrl->InsertItem(listCtrl->GetItemCount(), str0);
...
template <typename T>
void AddPendingCustomEvent(wxEvtHandler *pevthandler, int nEventID,
                           const T pbeginIn, const T pendIn) {
  if (!pevthandler)
    return;
...
void AddPendingReplyEvent1(void *pevthandler, CDataStream &vRecv) {
  if (IsCallbackAvailable(pevthandler))
    AddPendingCustomEvent((wxEvtHandler *)pevthandler, wxEVT_REPLY1,
                          vRecv.begin(), vRecv.end());
}

void AddPendingReplyEvent2(void *pevthandler, CDataStream &vRecv) {
  if (IsCallbackAvailable(pevthandler))
    AddPendingCustomEvent((wxEvtHandler *)pevthandler, wxEVT_REPLY2,
                          vRecv.begin(), vRecv.end());
}

void AddPendingReplyEvent3(void *pevthandler, CDataStream &vRecv) {
  if (IsCallbackAvailable(pevthandler))
    AddPendingCustomEvent((wxEvtHandler *)pevthandler, wxEVT_REPLY3,
                          vRecv.begin(), vRecv.end());
}

CDataStream GetStreamFromEvent(const wxCommandEvent &event) {
  wxString strData = event.GetString();
  return CDataStream(strData.begin(), strData.begin() + event.GetInt(),
                     SER_NETWORK);
}
...
CMainFrame::CMainFrame(wxWindow *parent) : CMainFrameBase(parent) {
  Connect(wxEVT_CROSSTHREADCALL,
          wxCommandEventHandler(CMainFrame::OnCrossThreadCall), NULL, this);
  ...
  //// shouldn't have to do mask separately anymore, bitmap alpha support added
  /// in wx 2.8.9,
  wxBitmap bmpSend(wxT("send20"), wxBITMAP_TYPE_RESOURCE);
  bmpSend.SetMask(
      new wxMask(wxBitmap(wxT("send20mask"), wxBITMAP_TYPE_RESOURCE)));
  m_toolBar->AddTool(wxID_BUTTONSEND, wxT("&Send Coins"), bmpSend, wxNullBitmap,
                     wxITEM_NORMAL, wxEmptyString, wxEmptyString);

  wxBitmap bmpAddressBook(wxT("addressbook20"), wxBITMAP_TYPE_RESOURCE);
  bmpAddressBook.SetMask(
      new wxMask(wxBitmap(wxT("addressbook20mask"), wxBITMAP_TYPE_RESOURCE)));
  m_toolBar->AddTool(wxID_BUTTONRECEIVE, wxT("&Address Book"), bmpAddressBook,
  ...
  m_listCtrl->InsertColumn(4, "Description", wxLIST_FORMAT_LEFT,
                           409 - nDateWidth);
  m_listCtrl->InsertColumn(5, "Debit", wxLIST_FORMAT_RIGHT, 79);
  m_listCtrl->InsertColumn(6, "Credit", wxLIST_FORMAT_RIGHT, 79);

  // m_listCtrlProductsSent->InsertColumn(0, "Category", wxLIST_FORMAT_LEFT,
  // 100); m_listCtrlProductsSent->InsertColumn(1, "Title", wxLIST_FORMAT_LEFT,
  // 100); m_listCtrlProductsSent->InsertColumn(2, "Description",
  // wxLIST_FORMAT_LEFT,  100); m_listCtrlProductsSent->InsertColumn(3, "Price",
  // wxLIST_FORMAT_LEFT,  100); m_listCtrlProductsSent->InsertColumn(4, "",
  // wxLIST_FORMAT_LEFT,  100);

  // m_listCtrlOrdersSent->InsertColumn(0, "Time",          wxLIST_FORMAT_LEFT,
  // 100); m_listCtrlOrdersSent->InsertColumn(1, "Price", wxLIST_FORMAT_LEFT,
  // 100); m_listCtrlOrdersSent->InsertColumn(2, "", wxLIST_FORMAT_LEFT,  100);
  // m_listCtrlOrdersSent->InsertColumn(3, "",              wxLIST_FORMAT_LEFT,
  // 100); m_listCtrlOrdersSent->InsertColumn(4, "", wxLIST_FORMAT_LEFT,  100);

  // m_listCtrlOrdersReceived->InsertColumn(0, "Time", wxLIST_FORMAT_LEFT, 100);
  // m_listCtrlOrdersReceived->InsertColumn(1, "Price", wxLIST_FORMAT_LEFT,
  // 100); m_listCtrlOrdersReceived->InsertColumn(2, "Payment Status",
  // wxLIST_FORMAT_LEFT,  100); m_listCtrlOrdersReceived->InsertColumn(3, "",
  // wxLIST_FORMAT_LEFT,  100); m_listCtrlOrdersReceived->InsertColumn(4, "",
  // wxLIST_FORMAT_LEFT,  100);
...
CMainFrame::~CMainFrame() { pframeMain = NULL; }
...
void CMainFrame::InsertLine(bool fNew, int nIndex, uint256 hashKey,
                            string strSort, const wxString &str2,
...
void CMainFrame::InsertTransaction(const CWalletTx &wtx, bool fNew,
                                   int nIndex) {
  ...
  string strSort =
      strprintf("%010d-%01d-%010u", (pindex ? pindex->nHeight : INT_MAX),
                (wtx.IsCoinBase() ? 1 : 0), wtx.nTimeReceived);
  ...
  if (nNet > 0 || wtx.IsCoinBase()) {
    ...
    if (wtx.IsCoinBase()) {
      ...
      if (nCredit == 0) {
        ...
        if (wtx.IsInMainChain())
          strDescription += strprintf(" (%s matures in %d blocks)",
                                      FormatMoney(nUnmatured).c_str(),
    ...
    InsertLine(fNew, nIndex, hash, strSort, strStatus,
               nTime ? DateTimeStr(nTime) : "", strDescription2, "",
    ...
    if (fAllFromMe && fAllToMe) {
      ...
      InsertLine(fNew, nIndex, hash, strSort, strStatus,
                 nTime ? DateTimeStr(nTime) : "", "Payment to yourself",
                 FormatMoney(nNet - nValue, true), FormatMoney(nValue, true));
    } else if (fAllFromMe) {
      //
      // Debit
      //
      ...
      for (int nOut = 0; nOut < wtx.vout.size(); nOut++) {
        ...
        if (mapAddressBook.count(strAddress) &&
            !mapAddressBook[strAddress].empty())
          strDescription += mapAddressBook[strAddress] + " ";
        strDescription += strAddress;
        if (!mapValue["message"].empty()) {
          if (!strDescription.empty())
        ...
        InsertLine(fNew, nIndex, hash,
                   strprintf("%s-%d", strSort.c_str(), nOut), strStatus,
                   nTime ? DateTimeStr(nTime) : "", strDescription2,
      ...
      InsertLine(fNew, nIndex, hash, strSort, strStatus,
                 nTime ? DateTimeStr(nTime) : "", "", FormatMoney(nNet, true),
                 "");
    }
  }
}
...
void CMainFrame::OnPaint(wxPaintEvent &event) { event.Skip(); }
...
void CMainFrame::OnPaintListCtrl(wxPaintEvent &event) {
      ...
      foreach (item, vWalletUpdated) {
        ...
        if (mi != mapWallet.end()) {
          printf("vWalletUpdated: %s %s\n",
                 (*mi).second.GetHash().ToString().substr(0, 6).c_str(),
                 fNew ? "new" : "");
  ...
  string strStatus =
      strprintf("     %d connections     %d blocks     %d transactions",
                vNodes.size(), nBestHeight + 1, m_listCtrl->GetItemCount());
...
void CMainFrame::OnMenuFileExit(wxCommandEvent &event) { Close(true); }
...
void CMainFrame::OnButtonCopy(wxCommandEvent &event) {
  ...
  if (wxTheClipboard->Open()) {
    wxTheClipboard->SetData(
        new wxTextDataObject(m_textCtrlAddress->GetValue()));
...
void CMainFrame::OnListItemActivatedAllTransactions(wxListEvent &event) {
  ...
  CRITICAL_BLOCK(cs_mapWallet) {
    map<uint256, CWalletTx>::iterator mi = mapWallet.find(hash);
    if (mi == mapWallet.end()) {
      printf("CMainFrame::OnListItemActivatedAllTransactions() : tx not found "
             "in mapWallet\n");
      return;
    }
    wtx = (*mi).second;
...
CTxDetailsDialog::CTxDetailsDialog(wxWindow *parent, CWalletTx wtx)
  ...
  if (wtx.IsCoinBase() && nCredit == 0) {
    ...
    if (wtx.IsInMainChain())
      strHTML +=
          strprintf("<b>Credit:</b> (%s matures in %d blocks)<br>",
                    FormatMoney(nUnmatured).c_str(), wtx.GetBlocksToMaturity());
    else
    ...
    if (fAllFromMe) {
      ...
      foreach (const CTxOut &txout, wtx.vout) {
        ...
        strHTML +=
            "<b>Debit:</b> " + FormatMoney(-txout.nValue) + " &nbsp;&nbsp; ";
        strHTML += "(to ";
        if (mapAddressBook.count(strAddress) &&
            !mapAddressBook[strAddress].empty())
          strHTML += mapAddressBook[strAddress] + " ";
      ...
      foreach (const CTxOut &txout, wtx.vout)
        if (txout.IsMine())
          strHTML +=
              "<b>Credit:</b> " + FormatMoney(txout.GetCredit()) + "<br>";
    }
  }
  ...
  if (!wtx.mapValue["message"].empty())
    strHTML += "<br><b>Message:</b><br>" +
               HtmlEscape(wtx.mapValue["message"], true) + "<br>";
        ...
        if (mi != mapWallet.end()) {
          ...
          if (prevout.n < prev.vout.size()) {
            ...
            strHTML = strHTML + "IsMine=" +
                      (prev.vout[prevout.n].IsMine() ? "true" : "false") +
                      "<br>";
          }
        }
      }
...
void COptionsDialog::OnButtonOK(wxCommandEvent &event) {
  ...
  if (ParseMoney(m_textCtrlTransactionFee->GetValue(), nTransactionFee) &&
      nTransactionFee != nPrevTransactionFee)
    CWalletDB().WriteSetting("nTransactionFee", nTransactionFee);

  Close();
}

void COptionsDialog::OnButtonCancel(wxCommandEvent &event) { Close(); }
...
CAboutDialog::CAboutDialog(wxWindow *parent) : CAboutDialogBase(parent) {
  m_staticTextVersion->SetLabel(
      strprintf("version 0.%d.%d Alpha", VERSION / 100, VERSION % 100));
...
void CAboutDialog::OnButtonOK(wxCommandEvent &event) { Close(); }
...
CSendDialog::CSendDialog(wxWindow *parent, const wxString &strAddress)
  ...
  bmpSend.SetMask(
      new wxMask(wxBitmap(wxT("send16masknoshadow"), wxBITMAP_TYPE_RESOURCE)));
  wxIcon iconSend;
...
void CSendDialog::OnTextAddress(wxCommandEvent &event) {
  ...
  m_textCtrlMessage->SetBackgroundColour(wxSystemSettings::GetColour(
      fEnable ? wxSYS_COLOUR_WINDOW : wxSYS_COLOUR_BTNFACE));
}
...
void CSendDialog::OnButtonSend(wxCommandEvent &event) {
  ...
  if (nValue + nTransactionFee > GetBalance()) {
    wxMessageBox(string("Total exceeds your balance when the ") +
  ...
  if (fBitcoinAddress) {
    ...
    scriptPubKey << OP_DUP << OP_HASH160 << hash160 << OP_EQUALVERIFY
                 << OP_CHECKSIG;
...
CSendingDialog::CSendingDialog(wxWindow *parent, const CAddress &addrIn,
                               int64 nPriceIn, const CWalletTx &wtxIn)
    : CSendingDialogBase(
          NULL) // we have to give null so parent can't destroy us
{
  addr = addrIn;
  nPrice = nPriceIn;
  wtx = wtxIn;
  ...
  SetTitle(strprintf("Sending %s to %s...", FormatMoney(nPrice).c_str(),
                     wtx.mapValue["to"].c_str()));
  m_textCtrlStatus->SetValue("");

  _beginthread(SendingDialogStartTransfer, 0, this);
}

CSendingDialog::~CSendingDialog() { printf("~CSendingDialog()\n"); }

void CSendingDialog::Close() {
  // Last one out turn out the lights.
  // fWorkDone signals that work side is done and UI thread should call destroy.
  // fUIDone signals that UI window has closed and work thread should call
  // destroy. This allows the window to disappear and end modality when
  // cancelled without making the user wait for ConnectNode to return.  The
  // dialog object hangs around in the background until the work thread exits.
  if (IsModal())
    EndModal(fSuccess);
  else
    Show(false);
...
void CSendingDialog::OnReply2(CDataStream &vRecv) {
  ...
  try {
    ...
    if (nRet > 0) {
      ...
      //// todo: enlarge the window and enable a hidden white box to put
      /// seller's message
      return;
    }
    vRecv >> scriptPubKey;
  ...
  CRITICAL_BLOCK(cs_main) {
    ...
    if (!CreateTransaction(scriptPubKey, nPrice, wtx, nFeeRequired)) {
      if (nPrice + nFeeRequired > GetBalance())
        Error(strprintf("This is an oversized transaction that requires a "
    ...
    // Send payment tx to seller, with response going to OnReply3 via event
    // handler
  ...
  try {
    ...
    if (nRet > 0) {
      Error("The payment was sent, but the recipient was unable to verify it.\n"
            "The transaction is recorded and will credit to the recipient if "
            "it is valid,\n"
...
CYourAddressDialog::CYourAddressDialog(wxWindow *parent,
                                       const string &strInitSelected)
    : CYourAddressDialogBase(parent) {
  // Init column headers
  ...
  CRITICAL_BLOCK(cs_mapKeys) {
    foreach (const PAIRTYPE(string, string) & item, mapAddressBook) {
      ...
      uint160 hash160;
      bool fMine =
          (AddressToHash160(strAddress, hash160) && mapPubKeys.count(hash160));
      if (fMine) {
        int nIndex = InsertLine(m_listCtrl, strName, strAddress);
        if (strAddress == strInitSelected)
          m_listCtrl->SetItemState(
              nIndex, wxLIST_STATE_SELECTED | wxLIST_STATE_FOCUSED,
...
void CYourAddressDialog::OnButtonRename(wxCommandEvent &event) {
  ...
  CGetTextFromUserDialog dialog(this, "Rename Bitcoin Address", "New Name",
                                strName);
  if (!dialog.ShowModal())
    return;
...
CAddressBookDialog::CAddressBookDialog(wxWindow *parent,
                                       const wxString &strInitSelected,
                                       bool fSendingIn)
  ...
  bmpAddressBook.SetMask(
      new wxMask(wxBitmap(wxT("addressbook16mask"), wxBITMAP_TYPE_RESOURCE)));
...
void CAddressBookDialog::OnButtonEdit(wxCommandEvent &event) {
  ...
  CGetTextFromUserDialog dialog(this, "Edit Address", "Name", strName,
                                "Address", strAddress);
  if (!dialog.ShowModal())
    return;
...
bool CompareIntStringPairBestFirst(const pair<int, string> &item1,
                                   const pair<int, string> &item2) {
  return (item1.first > item2.first);
}

CProductsDialog::CProductsDialog(wxWindow *parent)
    : CProductsDialogBase(parent) {
  // Init column headers
  ...
  for (map<uint256, CProduct>::iterator mi = mapProducts.begin();
       mi != mapProducts.end(); ++mi)
    mapTopCategories[(*mi).second.mapValue["category"]]++;

  // Sort top categories
  vector<pair<int, string>> vTopCategories;
  for (map<string, int>::iterator mi = mapTopCategories.begin();
       mi != mapTopCategories.end(); ++mi)
    vTopCategories.push_back(make_pair((*mi).second, (*mi).first));
  sort(vTopCategories.begin(), vTopCategories.end(),
       CompareIntStringPairBestFirst);

  // Fill categories combo box
  int nLimit = 250;
  for (vector<pair<int, string>>::iterator it = vTopCategories.begin();
       it != vTopCategories.end() && nLimit-- > 0; ++it)
...
void CProductsDialog::OnButtonSearch(wxCommandEvent &event) {
  ...
  foreach (CProduct *pproduct, vProductsFound) {
    InsertLine(m_listCtrl, pproduct->mapValue["title"],
               pproduct->mapValue["price"], pproduct->mapValue["seller"],
               pproduct->mapValue["stars"], itostr(pproduct->nAtoms));
  }
}

void CProductsDialog::OnListItemActivated(wxListEvent &event) {
  // Doubleclick opens product
  CViewProductDialog *pdialog =
      new CViewProductDialog(this, m_vProduct[event.GetIndex()]);
  pdialog->Show();
}
...
CEditProductDialog::CEditProductDialog(wxWindow *parent)
    : CEditProductDialogBase(parent) {
...
void CEditProductDialog::OnButtonDel0(wxCommandEvent &event) {
  OnButtonDel(event, 0);
}
void CEditProductDialog::OnButtonDel1(wxCommandEvent &event) {
  OnButtonDel(event, 1);
}
void CEditProductDialog::OnButtonDel2(wxCommandEvent &event) {
  OnButtonDel(event, 2);
}
void CEditProductDialog::OnButtonDel3(wxCommandEvent &event) {
  OnButtonDel(event, 3);
}
void CEditProductDialog::OnButtonDel4(wxCommandEvent &event) {
  OnButtonDel(event, 4);
}
void CEditProductDialog::OnButtonDel5(wxCommandEvent &event) {
  OnButtonDel(event, 5);
}
void CEditProductDialog::OnButtonDel6(wxCommandEvent &event) {
  OnButtonDel(event, 6);
}
void CEditProductDialog::OnButtonDel7(wxCommandEvent &event) {
  OnButtonDel(event, 7);
}
void CEditProductDialog::OnButtonDel8(wxCommandEvent &event) {
  OnButtonDel(event, 8);
}
void CEditProductDialog::OnButtonDel9(wxCommandEvent &event) {
  OnButtonDel(event, 9);
}
void CEditProductDialog::OnButtonDel10(wxCommandEvent &event) {
  OnButtonDel(event, 10);
}
void CEditProductDialog::OnButtonDel11(wxCommandEvent &event) {
  OnButtonDel(event, 11);
}
void CEditProductDialog::OnButtonDel12(wxCommandEvent &event) {
  OnButtonDel(event, 12);
}
void CEditProductDialog::OnButtonDel13(wxCommandEvent &event) {
  OnButtonDel(event, 13);
}
void CEditProductDialog::OnButtonDel14(wxCommandEvent &event) {
  OnButtonDel(event, 14);
}
void CEditProductDialog::OnButtonDel15(wxCommandEvent &event) {
  OnButtonDel(event, 15);
}
void CEditProductDialog::OnButtonDel16(wxCommandEvent &event) {
  OnButtonDel(event, 16);
}
void CEditProductDialog::OnButtonDel17(wxCommandEvent &event) {
  OnButtonDel(event, 17);
}
void CEditProductDialog::OnButtonDel18(wxCommandEvent &event) {
  OnButtonDel(event, 18);
}
void CEditProductDialog::OnButtonDel19(wxCommandEvent &event) {
  OnButtonDel(event, 19);
}
...
void CEditProductDialog::OnButtonCancel(wxCommandEvent &event) { Destroy(); }
...
CViewProductDialog::CViewProductDialog(wxWindow *parent,
                                       const CProduct &productIn)
    : CViewProductDialogBase(parent) {
  Connect(wxEVT_REPLY1, wxCommandEventHandler(CViewProductDialog::OnReply1),
          NULL, this);
  ...
  _beginthread(ThreadRequestProductDetails, 0,
               new pair<CProduct, wxEvtHandler *>(product, GetEventHandler()));
}
...
void ThreadRequestProductDetails(void *parg) {
  // Extract parameters
  pair<CProduct, wxEvtHandler *> *pitem =
      (pair<CProduct, wxEvtHandler *> *)parg;
  ...
  // Request detailed product, with response going to OnReply1 via dialog's
  // event handler
  pnode->PushRequest("getdetails", product.GetHash(), AddPendingReplyEvent1,
                     (void *)pevthandler);
}
...
void CViewProductDialog::UpdateProductDisplay(bool fDetails) {
  ...
  strHTML += "<html>\n"
             ...
             "<body>\n";
  strHTML +=
      "<b>Category:</b> " + HtmlEscape(product.mapValue["category"]) + "<br>\n";
  strHTML +=
      "<b>Title:</b> " + HtmlEscape(product.mapValue["title"]) + "<br>\n";
  strHTML +=
      "<b>Price:</b> " + HtmlEscape(product.mapValue["price"]) + "<br>\n";

  if (!fDetails)
    strHTML += "<b>Loading details...</b><br>\n<br>\n";
  else
    strHTML +=
        HtmlEscape(product.mapValue["description"], true) + "<br>\n<br>\n";
  ...
  if (!product.vchPubKeyFrom.empty()) {
    ...
    for (vector<CReview>::reverse_iterator it = vReviews.rbegin();
         it != vReviews.rend(); ++it) {
      CReview &review = *it;
      CUser user;
    ...
    // // 按原子数量排序（原子越多，评论越优先）
    stable_sort(vSortedReviews.begin(), vSortedReviews.end(),
                CompareReviewsBestFirst);
    ...
    foreach (CReview *preview, vSortedReviews) {
      ...
      // 根据信誉计算出最后的评分
      strHTML +=
          "<b>" + itostr(nStars) + (nStars == 1 ? " star" : " stars") + "</b>";
      strHTML += " &nbsp;&nbsp;&nbsp; ";
  ...
  for (int i = 0; i < product.vOrderForm.size(); i++) {
    ...
    m_staticTextLabel[i] =
        new wxStaticText(m_scrolledWindow, wxID_ANY, strLabel,
                         wxDefaultPosition, wxDefaultSize, wxALIGN_RIGHT);
    m_staticTextLabel[i]->Wrap(-1);
    fgSizer->Add(m_staticTextLabel[i], 0,
                 wxALIGN_CENTER_VERTICAL | wxALIGN_RIGHT | wxALL, 5);

    if (strControl.substr(0, 5) == "text=") {
      m_textCtrlField[i] =
          new wxTextCtrl(m_scrolledWindow, wxID_ANY, wxEmptyString,
                         wxDefaultPosition, wxDefaultSize, 0);
      fgSizer->Add(m_textCtrlField[i], 1,
      ...
      m_choiceField[i] =
          new wxChoice(m_scrolledWindow, wxID_ANY, wxDefaultPosition,
                       wxDefaultSize, arraystring, 0);
...
void CViewProductDialog::GetOrder(CWalletTx &wtx) {
  ...
  for (int i = 0; i < product.vOrderForm.size(); i++) {
    ...
    wtx.vOrderForm.push_back(
        make_pair(m_staticTextLabel[i]->GetLabel(), strValue));
  }
}
...
void CViewProductDialog::OnButtonSubmitForm(wxCommandEvent &event) {
  ...
  CSendingDialog *pdialog = new CSendingDialog(
      this, product.addr, atoi64(product.mapValue["price"]), wtx);
  if (!pdialog->ShowModal()) {
...
void CViewProductDialog::OnButtonCancel(wxCommandEvent &event) { Destroy(); }
...
CViewOrderDialog::CViewOrderDialog(wxWindow *parent, CWalletTx order,
                                   bool fReceived)
    : CViewOrderDialogBase(parent) {
  ...
  strHTML += "<html>\n"
             ...
             "<body>\n";
  strHTML +=
      "<b>Time:</b> " + HtmlEscape(DateTimeStr(order.nTimeReceived)) + "<br>\n";
  ...
  for (int i = 0; i < order.vOrderForm.size(); i++) {
    strHTML +=
        " <tr><td><b>" + HtmlEscape(order.vOrderForm[i].first) + ":</b></td>";
...
void CViewOrderDialog::OnButtonOK(wxCommandEvent &event) { Destroy(); }
...
CEditReviewDialog::CEditReviewDialog(wxWindow *parent)
    : CEditReviewDialogBase(parent) {}
...
void CEditReviewDialog::OnButtonCancel(wxCommandEvent &event) { Destroy(); }
...
bool CMyApp::OnInit2() {
  ...
  // Required to protect the database files if we're going to keep deleting
  // log.*
  //
  ...
  for (int i = 0; i < strMutexName.size(); i++)
    if (!isalnum(strMutexName[i]))
      strMutexName[i] = '.';
  wxSingleInstanceChecker *psingleinstancechecker =
      new wxSingleInstanceChecker(strMutexName);
  ...
  //
  {
    ...
    if (argc >= 2 && stricmp(argv[1], "/send") == 0) {
      ...
      // Send to IP address
      CSendingDialog *pdialog =
          new CSendingDialog(pframeMain, addr, nValue, wtx);
      if (!pdialog->ShowModal())
        return false;
    }
    ...
    if (mapArgs.count("/randsendtest")) {
      if (!mapArgs["/randsendtest"].empty())
        _beginthread(ThreadRandSendTest, 0,
                     new string(mapArgs["/randsendtest"]));
      else
        fRandSendTest = true;
      fDebug = true;
    }
...
void ThreadRandSendTest(void *parg) {
  ...
  if (!AddressToHash160(strAddress, hash160)) {
    wxMessageBox(
        strprintf("ThreadRandSendTest: Bitcoin address '%s' not valid ",
                  strAddress.c_str()));
    return;
  }
