...
public:
  ...
  CAddress(unsigned int ipIn, unsigned short portIn = 0,
           uint64 nServicesIn = NODE_NETWORK) {
    Init();
    ip = ipIn;
  ...
  bool SetAddress(string strIn) { return SetAddress(strIn.c_str()); }

  IMPLEMENT_SERIALIZE(if (fRead) const_cast<CAddress *>(this)->Init();
  ...
  string ToStringPort() const { return strprintf("%u", ntohs(port)); }

  string ToStringLog() const { return ""; }
...
public:
  ...
  void EndMessage() {
    ...
    if (vSend.GetVersion() >= 209) {
      ...
      memcpy((char *)&vSend[nHeaderStart] + offsetof(CMessageHeader, nChecksum),
